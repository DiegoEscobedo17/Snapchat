...
class ImagenViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBAction func elegirContactoTapped(_ sender: Any) {
        ...
        let cargarImagen = imagenesFolder.child("\(NSUUID().uuidString).jpg").putData(imagenData!, metadata: nil) { (metadata, error) in if error != nil{
        ...
        cargarImagen.observe(.progress) { (snapshot) in let porcentaje = Double(snapshot.progress!.totalUnitCount)
        print(porcentaje)
